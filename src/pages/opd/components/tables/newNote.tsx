...
function NewNote({ showConsultations, refetch, cid, onClose, setShowHistory }) {
  ...
  const { data, isLoading, mutate } = useWriteNotes();
  ...
  return (
    <div className=" m-2 p-3 rounded w-full">
      <form className="grid" onSubmit={handleSubmit}>
        <div className="grid gap-3">
          <div className="flex items-center justify-between w-full">
            <div className="flex items-center justify-start w-full gap-3 cursor-pointer">
              ...
              <label className="font-bold text-lg text-ha-primary1">
                Doctor's Notes:
              </label>
            </div>
            <div className="flex  w-full items-center justify-end gap-3">
              {showConsultations && (
                <div
                  className="w-[40%] text-white rounded p-2 bg-ha-primary1 flex items-center justify-center gap-3 cursor-pointer hover:bg-blue-600"
                  onClick={() => setShowHistory(true)}
                >
                  {" "}
                  <FaEye className="text-white text-2xl" /> View All
                  Consultations
                </div>
              )}
          ...
          {formData.notes.map((note, index) => (
            <div
              key={index}
              className="flex items-center w-full gap-3 bg-blue-50 p-5 rounded-lg my-2"
            >
        ...
        <button
          type="submit"
          className="bg-blue-600 my-2 p-3 rounded font-bold text-lg text-white hover:bg-gray-500"
        >
          Submit
        </button>
      </form>
