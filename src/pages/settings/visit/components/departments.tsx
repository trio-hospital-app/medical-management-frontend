...
function Departments() {
  ...
  const createDepartment = async () => {
    const data = { name: text, cost: parseFloat(cost) };
    try {
  ...
  return (
    <div className="w-full">
      {showCreate && (
        ...
        >
          <div className="grid">
            <label>Name Of Department e.g (Dentist)</label>
      ...
      <div className="rounded-[.5rem] py-5 px-5 grid md:grid-cols-3 gap-2">
        {departments?.data?.map((el) => (
          ...
          >
            ...
            <h5 className="text-sm font-semibold capitalize text-blue-900">
              NGN {el?.cost}
            </h5>
            ...
            <div className="w-full flex items-center justify-end gap-5">
              <FaEdit
                ...
                onClick={() => handleEdit(el?.id, el?.name, el?.cost)}
              />
              <MdDelete
